  ...
  return (
    <nav className="fixed top-0 w-full z-50 border-b border-white/5 bg-[#020617]/80 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
        ...
        {/* ── DROITE : AUTH + LANGUE + PROFIL ── */}
        <div className="hidden md:flex items-center gap-3">

          {/* Boutons Login / Signup — visibles SEULEMENT sans token */}
          {!isLoggedIn && (
            <div className="flex items-center gap-2">
              ...
              <Link
                to="/signup"
                className="bg-blue-600 hover:bg-blue-500 border border-blue-500 text-white px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 shadow-lg shadow-blue-500/20 active:scale-95"
              >
                {content.signup}
              </Link>
            </div>
          ...
          {/* Langue + Contact + Profil — toujours visibles */}
      ...
      {menuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-[#020617] border-b border-white/10 px-6 py-8 flex flex-col gap-6 shadow-2xl animate-in fade-in slide-in-from-top-4">
          ...
          <div className="flex flex-col gap-4">
            ...
            <NavItem to="/contact" label={content.contact} mobile />
            {/* Profil mobile — seulement si connecté */}
            {isLoggedIn && (
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* Login / Signup mobile — seulement si non connecté */}
            {!isLoggedIn && (
              <div className="grid grid-cols-2 gap-4">
                <Link
                  to="/signin"
                  onClick={() => setMenuOpen(false)}
                  className="py-4 rounded-2xl text-center font-bold text-white border border-white/10"
                >
                  {content.login}
                </Link>
                <Link
                  to="/signup"
                  onClick={() => setMenuOpen(false)}
                  className="bg-blue-600 text-white py-4 rounded-2xl text-center font-bold shadow-xl shadow-blue-600/20"
                >
                  {content.signup}
                </Link>
              </div>
